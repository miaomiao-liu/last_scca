...
oracleConnect=$oracle2_addr
oracleUser=$oracle2_user
oraclePwd=$oracle2_pwd
oracleDB=$oracle2_DB
oracleTbAllUpdate=('S_CLIENT' 'S_PROJECTCONFIG') ## The table in oracle need all first input
oracleTbIncUpdate=('S_EVIDENCE') ## The table in oracle need incremental first input
...
echo "`date "+%Y-%m-%d %H:%M:%S"` oracle data first input table end" >> ${oracleLog}
echo "==============================================================" >> ${oracleLog}
...
do  
    ...
    sqoop import --connect ${oracleConnect}/${oracleDB}  \
                 --username ${oracleUser} \
                 --password ${oraclePwd} \
                 -e "select * from ${tb} where to_char(CREATE_TIME,'yyyy-mm-dd hh24:mi:ss') <= '${end_time}' and to_char(CREATE_TIME,'yyyy-mm-dd hh24:mi:ss') >= '${start_time}' and \$CONDITIONS" \
                 -m 1 \
                 --target-dir ${hdfsDir}/${tb}/${date1day} \
                 --delete-target-dir \
                 --fields-terminated-by '\001' \
