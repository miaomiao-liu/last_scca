...
do
    ...
    sqoop import --connect "${oracle2Connect}/${oracle2DB}" \
            --username ${oracle2User} \
            --password ${oracle2Pwd} \
            -e "select count(*) from ${tb} where to_char(CREATE_TIME,'yyyy-mm-dd hh24:mi:ss') <= '${end_time}' and to_char(CREATE_TIME,'yyyy-mm-dd hh24:mi:ss') >= '${start_time}' and \$CONDITIONS" \
            -m 1 \
            --delete-target-dir \
            --target-dir ${hdfsDir}/${tb} \
            --fields-terminated-by '\001' \
