...
do
    ...
    sqoop import --connect "${oracle1Connect}/${oracle1DB}" \
            --username ${oracle1User} \
            --password ${oracle1Pwd} \
            -e "select count(*) from ${tb} where to_char(CREATE_DATE,'yyyy-mm-dd hh24:mi:ss') <= '${end_time}' and to_char(CREATE_DATE,'yyyy-mm-dd hh24:mi:ss') >= '${start_time}' and \$CONDITIONS" \
            -m 1 \
            --delete-target-dir \
            --target-dir ${hdfsDir}/${tb} \
            --fields-terminated-by '\001' \
